...
adversary_train = LinfPGDAttack(model, loss_fn=nn.CrossEntropyLoss().cuda(), eps=8 / 255,
                               nb_iter=40, eps_iter=0.01, rand_init=True, clip_min=0, clip_max=1.0,
                               targeted=False)

adversary_test = LinfPGDAttack(model, loss_fn=nn.CrossEntropyLoss().cuda(), eps=8 / 255,
